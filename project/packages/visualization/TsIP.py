...
class TsIP:
    ...
    def plot_df_level_1(self, graph, df, title_name = None):
        """
        ***Sub-plot function***
    ...
    def plot_df_level_2(self, name, graph, df):
        """
        ***Sub-plot function***
        ...
        self.plot_df_level_1(graph, group, title_name)
        
                
    def plot_df_level_3(self, name1, name2, graph, df):
        """
        ***Sub-plot function***
    ...
    def plot_df_level_4(self, name1, name2, name3, graph, df):
        """
        ***Sub-plot function***
    ...
    def interactive_plot_df(self, title = None, yaxis = None, style = "lines", first_last_valid_index_group = False, 
       ...
       """
        ...
        if not isinstance(self.df.columns, pd.MultiIndex):
            ...
            p = interact(self.plot_df_level_1, graph = w, df = fixed(self.df), title_name = fixed(self.title))
        else:
            # 2 LEVELS, AXIS 1.
            if len(self.df.columns.levels) == 2:
                ...
                if len(self.df.columns.get_level_values(1).unique()) == 1:
                    ...
                    # Create figure.
                    w = widgets.RadioButtons(options = ["Time-series", "Missing values"], description = "Select:", disabled = False)
                    p = interact(self.plot_df_level_1, graph = w, df = fixed(df), title_name = fixed(self.title))
                # Check if the dataframe has an unique feature on level 0 and multiple features on level 1.
                    ...
                    w = widgets.RadioButtons(options = ["Time-series", "Missing values"], description = "Select:", disabled = False)
                    p = interact(self.plot_df_level_2, name = fixed(self.df.columns.get_level_values(0).unique().values[0]), graph = w, df = fixed(self.df))
                # Check if dataframe has multiple features both on level 0 and level 1.    
                    ...
                    p = interact(self.plot_df_level_2, name = w1, graph = w2, df = fixed(self.df))       
            # 3 LEVELS, AXIS 1.
                ...
                if len(self.df.columns.get_level_values(2).unique()) == 1:
                    ...
                    w1 = widgets.ToggleButtons(options = df.columns.get_level_values(0).unique(), 
                                               ...
                                               disabled = False)
                    w2 = widgets.RadioButtons(options = ["Time-series", "Missing values"], description = "Select:", disabled = False)
                    p = interact(self.plot_df_level_2, name = w1, graph = w2, df = fixed(df))
                else:
                    # Create figure.
                    ...
                    out = widgets.interactive_output(self.plot_df_level_3, {"name1": w1, "name2": w2, "graph": w3, "df": fixed(self.df)})
                    display(hbox, w3, out)       
            # 4 LEVELS, AXIS 1.
                ...
                out = widgets.interactive_output(self.plot_df_level_4, {"name1": w1, "name2": w2, "name3": w3, "graph": w4, "df": fixed(self.df)})
                display(hbox, w4, out)    
