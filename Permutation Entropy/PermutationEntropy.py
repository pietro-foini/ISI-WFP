...
def PE(serie, m, normalization = "obs"):
    ...
    list_of_sorted_patterns_index = list(map(tuple, np.argsort(rolling_window(serie, m).T) + 1))
...
def PE_scaling_with_amount_of_data(serie, n_iter, ms, min_window_length = 10, max_window_length = 100):
    ...
    for iteration in range(n_iter):        
        ...
        summaries.append([entropy, n, best_m])

    summaries = pd.DataFrame(summaries, columns = ["PE", "n", "m"])
    summaries.columns.name = "Parameter"
