...
parser_user.add_argument('--folder_path_to_hyperparameters', type = str, help = "The path to the folder containing the results obtained from a previous hyperparameter tuning session. If defined, the forecasting is performed using the corresponding best configurations found, otherwise you can define a configuration through a GUI interface. In the last case, the xgboost parameters are set to their default values.")
parser_user.add_argument('--hyperparameters_session', type = str, help = "The name of the hyperparameter session to analyze (e.g. 'standard').")
...
parser_user.add_argument('--start_date', type = str, default = None, help = "The start date of target time-serie with which build the training points. If None all the time-serie is considered. This option is valid only if you don't use the results from a previous hyperparameter tuning.")
parser_user.add_argument('--gui_interface', action = "store_true", help = "If you want to select the time features and the lags for each indicator through a GUI interface otherwise the corresponding default values are taken (see *_default.py*). This option is valid only if you don't use the results from a previous hyperparameter tuning.")
...
os.makedirs(f"{args.folder_path_to_workspace}/features_selection")
os.makedirs(f"{args.folder_path_to_workspace}/features_importance")
...
df = pd.read_csv(f"{args.folder_path_to_dataset}/dataset.csv", header = [0, 1, 2], index_col = 0)
...
if args.folder_path_to_hyperparameters is not None:
    ...
    hyper_params = pd.read_csv(f"{args.folder_path_to_hyperparameters}/{args.hyperparameters_session}/hyperparameter_tuning.csv") 
    ...
    SPLITS_TO_CONSIDER = BEST_RESULTS.index.get_level_values("split").unique()
    # Load the values of start date.
    with open(f"{args.folder_path_to_hyperparameters}/start_date", "rb") as f:
        START_DATE = pickle.load(f)[0]
    # Load the lags dictionary defined during the hyperparameter tuning.
    with open (f"{args.folder_path_to_hyperparameters}/lags_dict", "rb") as fp:
        LAGS_DICT = pickle.load(fp)
    # Load the names of the parameters of the model.
    with open(f"{args.folder_path_to_hyperparameters}/space_model", "rb") as fp:
    ...
    with open(f"{args.folder_path_to_hyperparameters}/space_indicators", "rb") as fp:
    ...
    # Load the names of the provinces considered by the current hyperparameter analysis.
    with open(f"{args.folder_path_to_hyperparameters}/{args.hyperparameters_session}/provinces_considered", "rb") as fp:
    ...
    # GUI interface (it allows to modify default time features and lags selection).
    ...
    # Store information regarding hyperparameter tuning.
    HYPER = None
    # All the available provinces are considered for the current analysis.
    PROVINCES_CONSIDERED = None
...
for split_number, (train, test) in SPLITS.items():
    if split_number in SPLITS_TO_CONSIDER:
        ...
        # Keep only the provinces selected from a previous hyperparameter tuning session or all the provinces.
        if PROVINCES_CONSIDERED is not None:
            provinces = PROVINCES_CONSIDERED.copy()           
        else:
        ...
        columns = [(i,x) for i in provinces for x in provinces[i]]
        train = train.loc[:, train.columns.droplevel("Indicator").isin(columns)]
        test = test.loc[:, test.columns.droplevel("Indicator").isin(columns)]

        ## ACTUAL ##
        # Define the test points of the target to predict for each site (country and province).
        true = test.xs(TARGET, axis = 1, level = "Indicator", drop_level = False)
        ...
        naive = train.xs(TARGET, axis = 1, level = "Indicator", drop_level = False).iloc[-1].to_frame().transpose()
        ...
        predictions, models, r2_train, shape_train = model(train, test, LAGS_DICT, TEST_SIZE, TARGET, split_number, 
                                                           start_date = START_DATE, hyper = HYPER, 
        ...
        with open(f"{args.folder_path_to_workspace}/features_importance/importance_type.txt", "w") as fp:
            fp.write("Feature importance type: %s" % (args.importance_type))
        
        for h in models.keys():
            with open(f"{args.folder_path_to_workspace}/features_selection/features_split_{split_number}_h_{h}", "wb") as fp:
                pickle.dump(models[h][1], fp)
            with open(f"{args.folder_path_to_workspace}/features_importance/features_split_{split_number}_h_{h}", "wb") as fp:
...
excel_filename = f"{args.folder_path_to_workspace}/forecast.xlsx"
...
training_shapes.to_csv(f"{args.folder_path_to_workspace}/training_shapes.csv")
...
xls = pd.ExcelFile(f"{args.folder_path_to_workspace}/forecast.xlsx")
...
loss_overall.to_csv(f"{args.folder_path_to_workspace}/overall_loss.txt", sep = "\t")
print("Loss overall: \n", loss_overall)
...
loss_h.to_csv(f"{args.folder_path_to_workspace}/loss_h.csv")
...
r2_results.to_csv(f"{args.folder_path_to_workspace}/r2_results.csv")
